...
resource "github_repository" "myrepo-github" {
    name = "bookstore-api"
    auto_init = true
    visibility = "private"
}
...
resource "github_branch_default" "main" {
    branch = "main"
    repository = github_repository.myrepo-github.name
}
...
resource "aws_instance" "tf-docker-ec2" {
  ...
  user_data = <<-EOF
          ...
          FOLDER="https://$TOKEN@raw.githubusercontent.com/Rose94t/bookstore-api/main/"
          ...
          docker build -t havvanur94/bookstoreapi:latest .
          docker-compose up -d
          EOF
  depends_on = [github_repository.myrepo-github, github_repository_file.app-files]
}
